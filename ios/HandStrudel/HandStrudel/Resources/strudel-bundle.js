...
var _SAMPLE_INSTRUMENTS_CONFIG = [
...
async function loadSampleInstrumentManifests() {
  ...
  for (const entry of _SAMPLE_INSTRUMENTS_CONFIG) {
    (byManifest[_a2 = entry.manifest] || (byManifest[_a2] = [])).push(entry);
  }
...
function _getSampleBuffer(inst, midi2) {
  const choice = _findClosestSampledNote(inst, midi2);
  if (!choice) return null;
  const cached = Eo2(choice.url);
  if (cached) return { buffer: cached, sampledMidi: choice.midi };
  dt3(choice.url, _audioCtx, "instrument").catch((e) => log("loadBuffer error: " + e));
  return null;
}
...
function _startSampleVoice(hand, midi2, instName, vel) {
  ...
  const sampled = _getSampleBuffer(inst, midi2);
  if (!sampled) return true;
  const now = _audioCtx.currentTime;
  const v2 = vel || 0.6;
  const semitoneOffset = midi2 - sampled.sampledMidi;
  ...
  gain.connect(_audioCtx.destination);
  src.start(now);
  window._voices[hand] = { src, gain, midi: midi2, instName, kind: "sample", sampledMidi: sampled.sampledMidi };
  return true;
}
